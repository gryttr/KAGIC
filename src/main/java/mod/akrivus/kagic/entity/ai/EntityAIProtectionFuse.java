...
public class EntityAIProtectionFuse<P extends EntityGem, F extends EntityFusionGem> extends EntityAIBase {
	...
	public boolean shouldExecute() {
		...
		for (P gem : list) {
			if (this.checkInitiator() && this.checkTarget(gem) && this.initiator.getGemPlacement() != gem.getGemPlacement() && this.initiator.isOwnedBySamePeople(gem)) {
				double newDistance = this.initiator.getDistanceSqToEntity(gem);
	...
	public void updateTask() {
		if (this.initiator.getDistanceSqToEntity(this.target) > this.target.width * 3) {
	...
	public void resetTask() {
		this.initiator.getNavigator().clearPathEntity();
		this.target = null;
	}
	
	private boolean checkInitiator() {
		return !this.initiator.isFusion() && this.initiator.getHealth() > 0 && !this.initiator.isDead;
	}
