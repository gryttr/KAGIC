...
public class BlockAnalogInjector extends Block {
	public static final PropertyDirection FACING = BlockHorizontal.FACING;
	protected final boolean isEquipped;
	public BlockAnalogInjector(boolean isEquipped) {
		super(Material.GLASS, MapColor.MAGENTA);
		this.setUnlocalizedName("analog_injector");
	...
	public void injectGemSeed(World worldIn, BlockPos pos) {
		if (!(worldIn.getBlockState(pos.up()).getBlock() instanceof BlockIncubator)) {
			...
			InjectionEvent e1 = new InjectionEvent(worldIn, pos, newPosY, false);
		...
		if (!worldIn.isRemote) {
			...
			if (this.isEquipped && heldItem.isEmpty()) {
	    		...
	    		if (heldItem.getItem() == ModItems.ACTIVATED_GEM_BASE) {
	    			worldIn.setBlockState(pos, ModBlocks.EQUIPPED_ANALOG_INJECTOR.getDefaultState().withProperty(FACING, worldIn.getBlockState(pos).getValue(FACING)));
	...
	public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
		if (!worldIn.isRemote) {
			if (this.isEquipped && worldIn.getBlockState(pos).getBlock() != ModBlocks.ANALOG_INJECTOR) {
	...
	public Item getItemDropped(IBlockState state, Random rand, int fortune) {
        return Item.getItemFromBlock(ModBlocks.ANALOG_INJECTOR);
    }
	
	@Override
    public ItemStack getItem(World worldIn, BlockPos pos, IBlockState state) {
        return new ItemStack(ModBlocks.ANALOG_INJECTOR);
    }
